...
class Ads:
  ...
  def get_translation_frame(self) -> translation_frame_lib.TranslationFrame:
    ...
    for index, row in self._df.iterrows():
      ...
      for headline_index in range(0, _NUM_HEADLINES):
        ...
        # TODO(): Move logic that changes keyword insertion tag to
        # its own function(s) once the appropiate name and task is determined
        # for it. Should it be named mask_keyword_insertion_tag,
        # modify_keyword_insertion_tag, etc? Should it be a single function that
        # changes and reverts the tag or separate functions that carry out the
        # former.
        headline_keyword_insertion_keys = {}
        for match_index, match in enumerate(
            _KEYWORD_INSERTION_TAG_REGEX.finditer(headline)
        ):
          key = str(match_index)
          # Value that matches the keyword insertion tag key.
          group = match.groups()[0]
          # Replace matched key of keyword insertion tag with an alpha numeric
          # integer.
          headline = re.sub(group, key, headline)
          headline_keyword_insertion_keys[key] = group
        ...
        terms_with_metadata[headline].keyword_insertion_keys = (
            headline_keyword_insertion_keys
        )
      ...
      for description_index in range(0, _NUM_DESCRIPTIONS):
        ...
        # changes and reverts the tag or separate functions that carry out the
        # former.
        description_keyword_insertion_keys = {}
        for match_index, match in enumerate(
            _KEYWORD_INSERTION_TAG_REGEX.finditer(description)
        ):
          key = str(match_index)
          # Value that matches the keyword insertion tag key.
          group = match.groups()[0]
          # Replace matched key of keyword insertion tag with an alpha numeric
          # integer.
          description = re.sub(group, key, description)
          description_keyword_insertion_keys[key] = group
        ...
        terms_with_metadata[description].keyword_insertion_keys = (
            description_keyword_insertion_keys
        )
  ...
  def apply_translations(
    ...
    for _, row in translation_frame.df().iterrows():
      ...
      for key in keyword_insertion_keys:
        # Replace alpha numeric integer key of the keyword insertion tag with
        # the stored/original key. E.g. {0: Buy Now} -> {KeyWord: Buy Now}.
        # TODO(): Move logic that changes keyword insertion tag to
        # its own function(s) once the appropiate name and task is determined
        # for it. Should it be named mask_keyword_insertion_tag,
        # modify_keyword_insertion_tag, etc? Should it be a single function that
